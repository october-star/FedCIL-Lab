...
class CBDRAdaptiveReply(BaseMethod):
    """
    ...
    def __init__(
        ...
        *args,
        ...
        run_name: str = "cbdr_adaptive_reply",
        old_temp: float = 0.9,
        new_temp: float = 1.1,
    ...
    def _compose_train_dataset(
        ...
        # 防止下面除法报错，特殊情况直接返回
        if n_cur == 0 or n_rep == 0:
            return full, None

        # If adaptive replay is disabled, keep uniform replay sampling.
        # 如果没开或者没有KL分，退回到uniform
        if not self.adaptive_replay or not self.class_kl_scores:
            current_weights = [1.0 / n_cur] * n_cur
            replay_weights = [self.replay_sampling_mass / n_rep] * n_rep
            return full, current_weights + replay_weights
        ...
        for dataset in retained:
            for index in range(len(dataset)):
                item = dataset[index]
                replay_labels.append(int(item[-1]))

        # 安全检查
        if len(replay_labels) != n_rep:
            return full, None

        raw_class_weights: dict[int, float] = {}
        for label in set(replay_labels):
            score = max(float(self.class_kl_scores.get(label, 0.0)), 0.0)
            weight = 1.0 + self.adaptive_replay_gamma * score
            weight = max(
                self.adaptive_replay_min_weight,
                min(self.adaptive_replay_max_weight, weight),
            )
            raw_class_weights[label] = weight

        replay_raw_weights = [
            raw_class_weights.get(label, 1.0)
            for label in replay_labels
        ]

        replay_sum = sum(replay_raw_weights)
        if replay_sum <= 0:
            replay_weights = [self.replay_sampling_mass / n_rep] * n_rep
        else:
            replay_weights = [
                self.replay_sampling_mass * weight / replay_sum
                for weight in replay_raw_weights
            ]
