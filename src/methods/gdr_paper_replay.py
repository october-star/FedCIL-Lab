...
class LocalReplayGDRPaper(BaseMethod):
    ...
    def train(self) -> dict:
        ...
        for task_id in range(self.dataset_manager.num_tasks):
            ...
            # Add KD: save current model as next_task's teacher
    ...
    def _enforce_retained_capacity(self, client_id: int) -> None:
        ...
        for index, entry in enumerate(flattened):
            by_class.setdefault(int(entry["label"]), []).append(index)
        ...
        while len(kept_indices) < capacity and classes:
            next_classes = []
            for label in classes:
                ...
                # if class_entries:
                class_entry_indices = by_class[label]
                if class_entry_indices and len(kept_indices) < capacity:
                    kept_indices.append(class_entry_indices.pop(0))
                if class_entry_indices:
                    next_classes.append(label)
            classes = next_classes

        kept = [flattened[index] for index in sorted(kept_indices)]
